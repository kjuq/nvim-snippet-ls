...
local function get_snips_by_ft(filetype)
	local ftsnippets = require(config.snippet_path)

	local snips = {}
	snips = vim.tbl_deep_extend('error', snips, ftsnippets.all)
	if filetype and ftsnippets[filetype] then
		snips = vim.tbl_deep_extend('error', snips, ftsnippets[filetype])
	end
	return snips
end
...
function M.select()
	...
	local snippets = get_snips_by_ft(vim.bo.filetype)
	local candidates = {}
...
local function get_lsp_snippets(params)
	...
	for label, body in pairs(get_snips_by_ft(vim.bo.filetype)) do
